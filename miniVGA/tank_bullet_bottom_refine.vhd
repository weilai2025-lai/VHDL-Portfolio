...
entity tank_bullet_bottom_refine is
	...
	generic(
		...
		DIR_UP:std_logic:= '1'; --1:up 0: down
...
end entity tank_bullet_bottom_refine;

architecture behavior of tank_bullet_bottom_refine is
--------define state---------
		...
		when FLY =>
			if move_pulse = '1' then
				...
				if DIR_UP = '1' then
					...
					y_temp := to_integer(y_reg) + STEP_PIXELS;
					if y_temp > SCREEN_HEIGHT - BULLET_SIZE then
						state_next <= IDLE;
						active_next <= '0';
						fire_next <= '0';						
					else
